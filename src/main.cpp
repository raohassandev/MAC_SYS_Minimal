...
void setupDeviceWebServer() {
    ...
    device_server->on("/", []() {
        ...
        html += "  btn.style.background = '#1e40af'; btn.textContent = 'Loading...';";
        html += "  fetch('/api/system').then(r => r.json()).then(d => {";
        html += "    btn.style.background = '#10b981'; btn.textContent = 'Success!';";
        html += "    setTimeout(() => { btn.style.background = '#3b82f6'; btn.textContent = 'System API'; }, 2000);";
        ...
        html += "  }).catch(e => { btn.style.background = '#ef4444'; btn.textContent = 'Error!'; setTimeout(() => { btn.style.background = '#3b82f6'; btn.textContent = 'System API'; }, 2000); console.error(e); });";
        html += "}";
        html += "function testDiagnosticsAPI() {";
        html += "  const btn = event.target;";
        html += "  btn.style.background = '#1e40af'; btn.textContent = 'Testing...';";
        html += "  setTimeout(() => { btn.style.background = '#10b981'; btn.textContent = 'OK!'; setTimeout(() => { btn.style.background = '#6b7280'; btn.textContent = 'Diagnostics'; }, 2000); }, 1000);";
        html += "}";
        html += "function testNetworkAPI() {";
        html += "  const btn = event.target;";
        html += "  btn.style.background = '#1e40af'; btn.textContent = 'Pinging...';";
        html += "  fetch('/api/status').then(r => r.json()).then(d => {";
        html += "    btn.style.background = '#10b981'; btn.textContent = 'Connected!';";
        html += "    setTimeout(() => { btn.style.background = '#10b981'; btn.textContent = 'Network'; }, 2000);";
        html += "  }).catch(e => { btn.style.background = '#ef4444'; btn.textContent = 'Offline!'; setTimeout(() => { btn.style.background = '#10b981'; btn.textContent = 'Network'; }, 2000); });";
        html += "}";
        ...
        html += "  if (content.style.display === 'none') {";
        html += "    content.style.display = 'block';";
        html += "    arrow.textContent = '▼';";
        html += "    drawTempChart();";
        html += "  } else {";
        ...
        html += "  }";
        html += "}";
        html += "function drawTempChart() {";
        html += "  const canvas = document.getElementById('tempChart');";
        html += "  if (!canvas || tempHistory.length < 2) return;";
        html += "  const ctx = canvas.getContext('2d');";
        html += "  const width = canvas.width; const height = canvas.height;";
        html += "  ctx.clearRect(0, 0, width, height);";
        html += "  const minTemp = Math.min(...tempHistory) - 1; const maxTemp = Math.max(...tempHistory) + 1;";
        html += "  const tempRange = maxTemp - minTemp || 1;";
        html += "  ctx.strokeStyle = '#00D4FF'; ctx.lineWidth = 3; ctx.fillStyle = '#9CA3AF'; ctx.font = '12px Arial';";
        html += "  ctx.beginPath();";
        html += "  tempHistory.forEach((temp, i) => {";
        html += "    const x = (i / (tempHistory.length - 1)) * (width - 60) + 30;";
        html += "    const y = height - 30 - ((temp - minTemp) / tempRange) * (height - 60);";
        html += "    if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y);";
        html += "    ctx.fillRect(x - 2, y - 2, 4, 4);";
        html += "    if (i % 2 === 0) ctx.fillText(temp.toFixed(1) + '°', x - 15, y - 8);";
        html += "  });";
        html += "  ctx.stroke();";
